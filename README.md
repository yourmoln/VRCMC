...
字幕仅保留在窗口中，不写入聊天记录，不发送到 VRChat，也不触发朗读。捕获范围为启动时 Windows 默认播放设备上的全部声音（包括其他应用和本应用朗读），不按应用或说话人区分；切换播放设备后请重新开启监听。字幕延迟取决于句尾静音设置和服务响应速度。本地人声检测无需额外配置或联网下载；仅通过检测的语音片段会发送到配置的识别服务。
