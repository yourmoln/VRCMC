...
internal fun normalizeOutputOrder(languages: List<String>, configuredOrder: List<String>): List<String> {
    val selected = languages.filter { it.isNotBlank() }.distinct().take(2).ifEmpty { listOf("English") }
    ...
    valid.filterNot { it in normalized }.forEach { key ->
        val originalIndex = normalized.indexOf(originalOutputKey)
        if (key != originalOutputKey && originalIndex >= 0) normalized.add(originalIndex, key) else normalized.add(key)
    }
    return normalized
}
...
): String = outputOrder.mapNotNull { key ->
    when (key) {
