...
actual fun formatChatTime(timestamp: Long, now: Long, yesterdayLabel: String, dayBeforeYesterdayLabel: String): String {
    ...
    val prefix = when (messageDay) {
        today -> ""
        yesterday -> "$yesterdayLabel "
        dayBeforeYesterday -> "$dayBeforeYesterdayLabel "
        else -> return NSDateFormatter().apply { dateFormat = "yyyy-MM-dd HH:mm" }.stringFromDate(messageDate)
    }
...
private fun Long.toNSDate() = NSDate(timeIntervalSinceReferenceDate = this / 1000.0 - appleReferenceDateOffsetSeconds)
