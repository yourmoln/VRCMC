...
actual fun loadStoredTranslationSecrets(): String {
    ...
    return try {
        ...
        memScoped {
            ...
            try {
                if (CFGetTypeID(value) != platform.CoreFoundation.CFDataGetTypeID()) return@memScoped ""
...
actual fun saveStoredTranslationSecrets(value: String) {
    ...
    try {
        ...
        val data = encoded.usePinned { pinned -> CFDataCreate(null, pinned.addressOf(0).reinterpret(), encoded.size.convert()) } ?: return
        try {
...
private fun keychainQuery() = mutableDictionary()?.also { query ->
    ...
    val service = CFStringCreateWithCString(null, "io.github.vrcmteam.vrcmcs", kCFStringEncodingUTF8)
